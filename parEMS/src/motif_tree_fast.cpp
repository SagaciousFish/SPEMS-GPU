...
void MotifTreeFast::print() 
{
  std::cout << "root=" << root << ", name=" << name << std::endl;
  print_recursive(root);
}

void MotifTreeFast::print_recursive(const TreeNodeFast *t) 
{
  static int depth = 0;
...
void MotifTreeFast::deleteNode(TreeNodeFast* node) 
{
  assert(node);
...
bool MotifTreeFast::emptyNode(const TreeNodeFast* node) 
{
  for (size_t j=0; j<domain_size; j++) 
  {
    if (node->children[j]) 
...
//------ same as -----
void MotifTreeFast::setDomain(const string& _domain) 
{ 
  domain = _domain; 
...
void MotifTreeFast::traverse() 
{
  motifs.clear();
  (this)->traverseRecursive(root, 0);
}

void MotifTreeFast::traverseOut() 
{
...
void MotifTreeFast::insert(const std::string& motif) 
{
  (this)->insertRecursive(root, motif, 0);
}

void MotifTreeFast::intersect(const MotifTreeFast* other) 
{
