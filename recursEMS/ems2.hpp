...
void gen_nbrhood(int start, int delta, int sigma, int alpha) {
  ...
  void search() 
  {
    // printf("Thread %d start working on Seq %d ...\n", sched_getcpu(), mTarget);
    std::cout.flush();
    // std::cout << (int)reads[mTarget].at(0) << " " << reads[mTarget].size() << std::endl;
    gen_all(reads[mTarget], l, d);
    //printf("Thread %d generate All on Seq %d ...\n", mTarget, motifs.size());

    for (int i=mTarget+1; i<mTarget+2; i++) 
    {
      int cur_run = i;
      if (i >= reads.size())
        cur_run = mTarget-1;
      ...
      //std::cout << domain.size() << " " << domain << std::endl;
      curr_tree = &tmp_tree;
      gen_all(reads[cur_run], l, d);
